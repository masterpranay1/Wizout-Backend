...
exports.createPost=async(req,res)=>{
    try{
        ...
        if(images && visibility){
            const post=await PostModel.create({title,content,author,visibility,images});
        }
        else if(visibility){
            const post=await PostModel.create({title,content,author,visibility});
        }else{
            if(images){
...
exports.doUpvote=async(req,res)=>{
    try{
...
exports.doDownvote=async(req,res)=>{
    try{
        const post=await PostModel.findById(req.body._id);
        if(!post){
          res.status(404).json({
...
exports.commetOnPost=async(req,res)=>{
    try{
        const post=await PostModel.findById(req.body._id);
